...
sema_up (struct semaphore *sema) {
	...
	if (!list_empty (&sema->waiters)){
		...
		unblock_thread->wait_on_lock = NULL;
...
lock_acquire (struct lock *lock) {
	...
	//PDG 전체 쓰레드의 우선순위를 현재 쓰레드 우선순위로 변경하기 위함
	struct thread* cur;
	/*PDG START*/
	/* 락이 타 쓰레드에 걸려있는 경우 */
	if (lock->holder != NULL) {
	...
	int testP4 = thread_current()->priority;
	/*PDG END*/
	...
	*/
	...
	testP4 = thread_current()->priority;
}
...
lock_release (struct lock *lock) {
	...
	// 	lock->holder->priority = max_value;
	// }
